...
采用 **C++17** 和 **Qt 6.9.3** 开发，基于 **CMake** 构建，底层数据存储依赖 **MySQL 8.0**，是一套专为校园场景打造的物联网解决方案。
...
* **内置使用指南**：说明借还规则与收费标准，帮助新用户快速上手使用自助终端。

#### 管理端 / Management Dashboard

* **全局监控**：实时监控所有站点的在线状态与库存水位，辅助调度决策。
* **站点及雨具生命周期管理**：支持站点、雨具的状态变更。
* **用户账号与安全管理**
    * **状态审计**：支持管理员查看用户账号的**激活状态**（已激活/待激活）。
    * **应急安全运维**：提供管理员权限的**密码重置接口**。当用户忘记密码或账号异常时，管理员可在后台手动修改用户密码，确保账号的可控性与安全性。
* **数据统计**：可视化展示订单流水。
